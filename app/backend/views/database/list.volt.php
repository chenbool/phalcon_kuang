...
<div class="page-title">
	<div class="page-breadcrumb">
		<ol class="breadcrumb">
			...
			<li><a href="<?= $this->url->getStatic('/admin/database/list') ?>">备份列表</a></li>
		</ol>
	</div>
</div>
						...
						<div class="col-md-12">
                            <section class="panel">
                                ...
                                <header class="panel-heading">
                                    <a href="<?= $this->url->getStatic('/admin/database/index') ?>">
                                                ...
                                                <?php foreach ($list as $vo) { ?>
                                                    <tr <?= ($vo->state == 1 ? ' class="bg-danger" ' : null) ?>>
                                                        ...
                                                        <td style="text-align:center;">                                                          
                                                            <a href="<?= $this->url->getStatic('/admin/database/down?file=') ?><?= $vo ?>" class="btn btn-xs btn-warning"> 下载 </a> &nbsp;&nbsp;
    														<a href="javascript:;" class="btn btn-xs btn-danger"  onclick="del('<?= $vo ?>')"> 删 除 </a> 
                                                            <!-- <?= $this->url->getStatic('/admin/database/rm?file=') ?><?= $vo ?> -->
                                                        </td>
                                                    </tr>             
                                                <?php } ?>
...
function del(file){
    ...
    layer.confirm('你真的要删除?', {icon: 3, title:'提示'}, function(index){
        //do something
        $.post('<?= $this->url->getStatic('/admin/database/rmfile') ?>',{file:file},function(res){
            ...
            if( res.status > 0 ){
                ...
                layer.msg( res.info[0] )
                setTimeout(function(){
                    location.href = '<?= $this->url->getStatic('/admin/database/list') ?>';
                },1000);
            }
